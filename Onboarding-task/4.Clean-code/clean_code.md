...
2. Find and example of poorly commented code and rewrite the comments to be more useful
   ...
   - Rewrite the comments to be more useful :
     ...
     Why: - Used by the budgeting screen to apply a fixed offset equal to sum(0..N-1).
     Contracts: - a, b are integers (can be negative). - N defaults to 100; change with env var `OFFSET_N` if needed.
     _/
     export function sumWithOffset(a: number, b: number, N = Number(process.env.OFFSET*N ?? 100)) {
     const base = a + b
     // Triangular number T*(N-1) = N*(N-1)/2 (faster than a loop)
...
1. Refactor it to make it simpler and more readable
    // cart.js
    /\*\*
    _@typedef {Object} CartItem
    _@property {number} product_id
    _@property {string} name
    _@property {number} price
    _@property {number} quantity
    _/
...
3. Refactor the code by renaming variables/functions for better clarity.
    // cart-utils.js
    /\*\*
    _Calculate a cart total after discount and tax.
    _@param {{price:number, quantity:number}[]} items
    _@param {string} discountCode
    _@param {number} [taxRate=0.1]
    */
...
In the world of software development, especially in large projects, code consistency is not just a good-to-have; it’s a necessity. When multiple developers collaborate on a project, having consistent code helps ensure that everyone can understand, maintain, and extend the codebase efficiently. Inconsistent code can lead to confusion, bugs, and a significant increase in the time it takes to onboard new team members.

1. Review the Airbnb javascript style guide.
![Reviewing the style guide](review_jsairbnb.png)

1. Install and configure ESlint and Prettier in your development environment

- **Installation step**:
![Step 1 for install and configure](step1_js_style.png)

- **Configuration step**:
![Step 2 for configuration](step2.1_js.png)
![Step 2 for configuration](step2.2_js.png)
![Step 2 for configuration](step2.3_js.png)
![Step 2 for configuration](step2.4_js.png)
![Step 2 for configuration](step2.5_js.png)
![Step 2 for configuration](step2.6_js.png)

1. Run the formatter and linter on your codebase and fix any issues.
![Run the formatter and linter's result](step3.1_js.png)
![Run the formatter and linter's result](step3.2_js.png)
![Run the formatter and linter's result](step3.3_js.png)
![Run the formatter and linter's result](step3.4_js.png)

### Reflection - code formatting

1. Why is code formatting important?
    - Shared readability: a consistent style makes files look the same no matter who wrote them, so reviews are faster and cognitive load is lower.
    - Fewer merge conflicts: normalized whitespace (final newline, no trailing spaces, consistent wrapping) reduces noisy diffs.
    - Focus on substance: tools decide spacing/quotes/commas so humans can discuss design and logic, not style.
    - Automatable quality gate: with CI + pre-commit hooks, formatting becomes a guaranteed baseline instead of a “nice to have.”.

1. What issues did the linter detect?

Most of this repo is Markdown, so the majority of issues were formatting, not logic:

- Markdown spacing rules:
MD030 list-marker-space – some list markers had two spaces after -/1. instead of exactly one.
MD009 no-trailing-spaces – stray spaces at end of lines.
MD047 single-trailing-newline – missing a newline at file end.
- Prettier “needs formatting” on many files (Markdown/YAML/JSON/JS), which it fixed automatically (indentation, wrapping, quotes, commas, final newline).
- ESLint (Airbnb base) minor style warnings in JS files (e.g., prefer const when a binding isn’t reassigned, enforce ===, trailing commas/quotes/semicolons)
