...
1. Observations from React's Commit History :
   ...
   - Scoped and tagged : It starts with a tag (e.g. [Flight], [compiler],..) which shows the part of the project affected.
...
3. How does a clear commit message help in team collaboration?
   ...
   - When an issue appears, you can pinpoint the exact change that caused it.
...
3. Git log
   ...
   - Test result : I used 'git log' to visualize branches and commits.
...
### Reflection - Advanced git command
...
3. What happens if two people edit the same file on different branches?
   ...
   - The changes exists independently until somebody creates a merge or a pull request.
...
### Reflection - Git concept

1. What is the difference between staging and committing?
   - Staging (git add) will transfer the altered files to the staging area which is a staging ground where you arrage precisely what you want to be committed in the next commit.
   ...
   - An example in my experiment:
     - Once staged test.txt, it appeared in green and ready to be committed to the repository on git status.
...
3. When would you want to stage changes without committing?
   - When you are deploying larger features and you need/feel you want to stash-off a partial work but still going to edit and complete what you need to do before committing.
